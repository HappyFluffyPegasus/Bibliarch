          ...
          {viewportNodes.filter(node => !node.parentId).map(node => {
            ...
            <div
              ...
              onMouseDown={(e) => {
                ...
                if (!isTextElement) {
                  // For folder nodes, completely skip our mouse-based movement
                  // Let HTML5 drag-and-drop handle them exclusively
                  if (node.type === 'folder') {
                    return // Don't interfere with HTML5 drag at all
                  }

                  // For all other node types, use mouse-based movement
                  e.preventDefault()
                  e.stopPropagation()
                  setDragStartPos({ x: e.clientX, y: e.clientY })
                  setIsDragReady(node.id)
