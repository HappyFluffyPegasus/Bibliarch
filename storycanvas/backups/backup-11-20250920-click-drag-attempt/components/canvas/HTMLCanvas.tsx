...
export default function HTMLCanvas({ 
  ...
  const [folderDragTimeout, setFolderDragTimeout] = useState<NodeJS.Timeout | null>(null)
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent) => {
    ...
    const newNode: Node = {
      ...
      width: tool === 'list' ? 320 : tool === 'image' ? 300 : 200,  // Image nodes bigger, list containers wider
      height: tool === 'list' ? 240 : tool === 'image' ? 200 : 120, // Image nodes taller, list containers taller
      type: tool,
  ...
  const handleCanvasMouseUp = useCallback(() => {
    ...
    // Clear any pending folder drag timeout
    if (folderDragTimeout) {
      clearTimeout(folderDragTimeout)
      setFolderDragTimeout(null)
    }
  ...
  }, [isMoving, draggingNode, isDragReady, resizingNode, nodes, connections, saveToHistory, dragPosition, folderDragTimeout])
  ...
  const handleDragStart = (e: React.DragEvent, nodeId: string) => {
    // Cancel any pending mouse-based drag timeout for folders
    if (folderDragTimeout) {
      clearTimeout(folderDragTimeout)
      setFolderDragTimeout(null)
    }
          ...
          {viewportNodes.filter(node => !node.parentId).map(node => {
            ...
            if (node.type === 'image') {
              return (
                <div
                  ...
                  className={`absolute cursor-move node-background ${
            ...
            <div
              ...
              onMouseDown={(e) => {
                ...
                if (!isTextElement) {
                  // For folder nodes, use a hybrid approach: start with mouse-based movement,
                  // but allow HTML5 drag-and-drop to take over if no immediate movement
                  if (node.type === 'folder') {
                    // Start our mouse tracking but with a delay to allow HTML5 to activate
                    const timeout = setTimeout(() => {
                      ...
                      setFolderDragTimeout(null)
                    }, 150) // 150ms delay to allow HTML5 drag to start first

                    setFolderDragTimeout(timeout)
                    return // Don't prevent default - let HTML5 drag work
                  }

                  // For all other node types, use immediate mouse-based movement
                  e.preventDefault()
                  e.stopPropagation()
                  setDragStartPos({ x: e.clientX, y: e.clientY })
                  setIsDragReady(node.id)
            ...
            >
              ...
              onClick={(e) => {
                              ...
                              onMouseDown={(e) => {
                                ...
                                // Only start drag if NOT clicking on editable text
                                if (!isTextElement) {
                                  // Child folder nodes in lists use the same hybrid approach
                                  ...
                                  // For non-folder child nodes, use immediate mouse-based movement
                                  e.preventDefault()
                                  e.stopPropagation()
                                  setDragStartPos({ x: e.clientX, y: e.clientY })
                                  setIsDragReady(childNode.id)
