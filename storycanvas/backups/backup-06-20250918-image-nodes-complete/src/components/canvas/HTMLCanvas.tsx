            ...
            >
              ...
              {nodeDetails.showContent && (
                ...
                {node.type === 'list' ? (
                  ...
                  // Simple image node - just image with colored outline
                  <div
                    className="w-full h-full cursor-pointer relative rounded-lg overflow-hidden"
                    style={{
                      border: `3px solid ${getNodeColor(node.type, node.color, node.id)}`,
                        ...
                        if (file) {
                          ...
                          reader.onload = (event) => {
                            ...
                            tempImg.onload = () => {
                              ...
                              // Simple calculation - just use natural size with 1.3x scaling
                              let imageWidth = Math.min(400, naturalWidth)
                              let imageHeight = Math.min(400, naturalHeight)
                              ...
                              // Make outer node fit the image frame with space for top icons
                              // Width: exact fit (image + 3px border each side)
                              const nodeWidth = Math.round(imageWidth + 6)
                              // Height: 1.2x scaling to account for icons at top
                              const nodeHeight = Math.round((imageHeight + 6) * 1.2)

                              console.log('Calculated node size:', nodeWidth, 'x', nodeHeight)
                  ...
                  >
                    {node.imageUrl ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center text-center">
                        <div
                          style={{
                            color: 'color-mix(in srgb, var(--node-border-default, hsl(var(--border))) 75%, transparent)'
                          }}
                        >
                          <div className="text-4xl mb-2">🖼️</div>
                          <div className="text-sm mb-1">Click to upload</div>
                          <div className="text-xs opacity-75">Image files</div>
                        </div>
                      </div>
                    )}
                  </div>
