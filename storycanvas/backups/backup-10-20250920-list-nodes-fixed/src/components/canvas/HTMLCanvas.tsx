        ...
        >
          ...
          {viewportNodes.filter(node => !node.parentId).map(node => {
            ...
            <div
              key={node.id}
              data-node-id={node.id}
              className={`absolute border-2 rounded-lg p-3 cursor-move hover:shadow-lg shadow-sm ${
                selectedId === node.id ? 'ring-2 ring-primary' : ''
              } ${
                connectingFrom === node.id ? 'ring-2 ring-orange-500' : ''
              } ${
              ...
              onMouseDown={(e) => handleNodeMouseDown(node, e)}
              onDoubleClick={() => handleNodeDoubleClick(node)}
              // Add HTML5 drag-and-drop for folder nodes (so they can be dropped into lists)
              {...(node.type === 'folder' ? {
                draggable: true,
                onDragStart: (e: React.DragEvent) => handleDragStart(e, node.id)
              } : {})}
              // Add HTML5 drag-and-drop for list nodes (so they can accept folder drops)
              {...(node.type === 'list' ? {
              ...
              <div className="flex items-center justify-between mb-2 gap-2">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <div
                    ...
                    onFocus={(e) => {
                      // Initialize content if empty and node has text
                      if (!e.currentTarget.textContent && node.text) {
                        e.currentTarget.textContent = node.text
                      }
                    }}
                    spellCheck={false}
                  >
              ...
              {nodeDetails.showContent && (
                ...
                {node.type === 'list' ? (
                        ...
                        {childNodes.map((childNode, index) => (
                          ...
                          >
                            ...
                            <div
                              ...
                              onMouseDown={(e) => {
                                handleNodeMouseDown(childNode, e)
                              }}
                              onDoubleClick={(e) => {
                                e.stopPropagation()
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between p-2 pb-1">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  ...
                                  <div
                                    ...
                                    onFocus={(e) => {
                                      // Initialize content if empty and node has text
                                      if (!e.currentTarget.textContent && childNode.text) {
                                        e.currentTarget.textContent = childNode.text
                                      }
                                    }}
                                    spellCheck={false}
                              ...
                              <div className="px-2 pb-2">
                                <div
                                  ...
                                  onFocus={(e) => {
                                    // Initialize content if empty and node has content
                                    if (!e.currentTarget.textContent && childNode.content) {
                                      e.currentTarget.textContent = childNode.content
                                    }
                                  }}
                                  spellCheck={false}
                  ...
                  <div
                    ...
                    onFocus={(e) => {
                      // Initialize content if empty and node has content
                      if (!e.currentTarget.textContent && node.content) {
