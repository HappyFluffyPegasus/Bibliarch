...
export default function HTMLCanvas({ 
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    if (isPanning) {
      ...
      if (resizingNodeObj.type === 'image') {
        // Image nodes: always maintain aspect ratio
        const originalWidth = resizingNodeObj.attributes?.originalWidth || resizeStartSize.width
        const originalHeight = resizingNodeObj.attributes?.originalHeight || resizeStartSize.height
        const aspectRatio = originalWidth / originalHeight

        // Resize based on whichever dimension changed more
        const widthChangeFactor = newWidth / resizeStartSize.width
        const heightChangeFactor = newHeight / resizeStartSize.height
  ...
  const calculateAutoSize = (node: Node, content: string) => {
    ...
    if (node.type === 'text' || !node.type) {
      ...
      // Image nodes: auto-scale to correct ratio of image, always maintain aspect ratio
      if (node.imageUrl && node.attributes?.originalWidth && node.attributes?.originalHeight) {
        const originalWidth = node.attributes.originalWidth
        const originalHeight = node.attributes.originalHeight
        const aspectRatio = originalWidth / originalHeight

        // If user manually resized, maintain their width and adjust height to preserve ratio
        if (node.width && node.width !== baseWidth) {
          const userWidth = Math.min(maxWidth, Math.max(minWidth, node.width))
          const calculatedHeight = userWidth / aspectRatio
          return {
            width: userWidth,
            height: Math.min(maxHeight, Math.max(minHeight, calculatedHeight))
          }
        }

        // Auto-scale to fit image naturally within reasonable bounds
        let newWidth = Math.min(maxWidth, Math.max(minWidth, originalWidth))
      ...
      // List nodes: auto-scale based on content within it
      const childCount = node.childIds?.length || 0
      const headerHeight = 40
      const childItemHeight = 32 // Height per child item in the list display
      const padding = 16
      ...
      // When list is scaled by user, scale all children proportionally (handled elsewhere)
      const newHeight = Math.min(maxHeight, Math.max(minListHeight, node.height || minListHeight))
      const newWidth = Math.min(maxWidth, Math.max(minListWidth, node.width || minListWidth))
            ...
            >
                ...
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <div
                    ...
                    onInput={(e) => {
                      ...
                      // Real-time auto-resize for title
                    ...
                    {childNodes.length > 0 ? (
                      <div className="space-y-1">
                        <div className="text-xs font-medium text-muted-foreground mb-1">
                          Contains {childNodes.length} folder{childNodes.length !== 1 ? 's' : ''}:
                        </div>
                      ...
                      <div className="text-center text-muted-foreground py-4">
                        ...
                        <div className="text-xs mt-1 text-yellow-600">⚠ Only accepts folder nodes</div>
                      </div>
                    )}
                  </div>
                ) : node.type === 'image' ? (
                    ...
                    {node.imageUrl ? (
                      <div className="flex-1 flex items-center justify-center p-2">
                        <img
                          ...
                          style={{
                            maxWidth: '100%',
                            maxHeight: 'calc(100% - 60px)', // Leave space for any caption/controls
                          }}
                          onLoad={(e) => {
                            ...
                            // Auto-resize to fit image with aspect ratio
                            const newSize = calculateAutoSize({ ...node, attributes: { ...node.attributes, originalWidth: naturalWidth, originalHeight: naturalHeight } }, '')
                            const resizedNodes = updatedNodes.map(n =>
                              n.id === node.id ? { ...n, width: newSize.width, height: newSize.height } : n
                            )
                      ...
                      <div className="flex-1 flex items-center justify-center text-center p-4">
                        <div className="text-muted-foreground">
                          <div className="text-sm mb-2">No image</div>
                          <div className="text-xs">Click to add image URL in content area below</div>
                        </div>
                      </div>
                    )}

                    {/* Image caption/description area */}
                    <div
                      contentEditable
                      suppressContentEditableWarning={true}
                      data-content-type="content"
                      className="w-full bg-transparent border-none outline-none text-xs p-1 cursor-text border-t border-border/30 min-h-[2rem]"
                      style={{
                        color: getTextColor(getNodeColor(node.type || 'text', node.color, node.id)),
                        caretColor: getTextColor(getNodeColor(node.type || 'text', node.color, node.id))
                      }}
                      onInput={(e) => {
                        ...
                        // If content looks like a URL, try to set it as imageUrl
                        if (newContent.trim().match(/^https?:\/\/.+\.(jpg|jpeg|png|gif|webp|svg)$/i)) {
                          const urlUpdatedNodes = updatedNodes.map(n =>
                            n.id === node.id ? { ...n, imageUrl: newContent.trim() } : n
                          )
                          setNodes(urlUpdatedNodes)
                        }

                        // Auto-resize image node caption area
                        const captionElement = e.currentTarget
                        const captionHeight = Math.max(32, captionElement.scrollHeight)
                        const updatedNode = updatedNodes.find(n => n.id === node.id)
                        if (updatedNode) {
                          setTimeout(() => {
                            // Keep image area the same, just adjust for caption
                            const imageAreaHeight = updatedNode.height - 50 // Estimate current image area
                            const newTotalHeight = Math.max(120, imageAreaHeight + captionHeight + 20)
                            const resizedNodes = updatedNodes.map(n =>
                              n.id === node.id ? { ...n, height: newTotalHeight } : n
                            )
                            setNodes(resizedNodes)
                          }, 0)
                        }
                      ...
                      spellCheck={false}
                      data-placeholder="Image caption or paste image URL..."
                    >
                    </div>
                  </div>
                ) : (
              ...
              {selectedId === node.id && tool === 'select' && !node.parentId && (
                <>
                  ...
                  {/* Right edge resize handle (width only) - not for image nodes */}
                  {node.type !== 'image' && (
                    <div
                  ...
                  {/* Bottom edge resize handle (height only) - not for image nodes */}
                  {node.type !== 'image' && (
                    <div
              ...
              {selectedId === node.id && node.parentId && (
                <div className="absolute -top-6 left-0 text-xs text-yellow-600 bg-background border border-yellow-600 rounded px-1">
